...
- v1.2.0（2026-09-16）：溯源核验报告对齐公文写作 3.7.0 原型重构——单栏文档流 + 行内引文胶囊（点击原地展开溯源卡）、材料专库独立视图（检索分组 tabs/搜索/热词/未引用筛选）、章节引用徽章、核验报告单与过程回顾（数字真实计算）、顶栏只看正文/复制全文/打印归档、生成时原文链接质量检测（404/410 判失效）与快照兜底（screenShotPath）。
